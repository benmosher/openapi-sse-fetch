...
import type { GetMessagesStreamParams, GetMessagesStreamEvent, GetMessagesStreamMessageEvent, GetMessagesStreamStatusEvent } from './types';
  ...
  fetchEventSource(_url, {
    ...
    onmessage(msg) {
      if (msg.event === "message") {
        ch.push({ event: "message", data: JSON.parse(msg.data), id: msg.id } as GetMessagesStreamMessageEvent);
        return;
      }
      if (msg.event === "status") {
        ch.push({ event: "status", data: JSON.parse(msg.data), id: msg.id } as GetMessagesStreamStatusEvent);
        return;
      }
    },
    onclose() { ch.done(); },
